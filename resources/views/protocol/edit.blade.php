                ...
                <div class="col-md-8 col-md-offset-2">
                    <div class="c_panel">
                        <div class="c_title mb-4">
                            <h2 class="m-0 font-weight-bold">Edit Protocol: <span
                                    class="text-primary">{{ $protocol->name }}</span></h2>
                        </div>
                        <div class="section-card">
                            <div class="section-header">
                                <h4><i class="fa fa-edit"></i> Protocol Identity</h4>
                            </div>
                            <div class="section-body">
                                <form class="premium-form" method="POST"
                                    action="{{ route('protocols.update', $protocol->id) }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group mb-4">
                                        ...
                                        <input type="text" name="name" class="form-control premium-input"
                                            value="{{ $protocol->name }}" required>
                                    </div>
                                    <div class="form-group mb-4">
                                        <label class="premium-label">Description (Optional)</label>
                                        <textarea name="description" class="form-control premium-textarea"
                                            rows="3">{{ $protocol->description }}</textarea>
                                    </div>
