      ...
      <div class="col-md-12">
        <div class="vt-card">
          ...
          <div class="vt-card-body">
            ...
            @foreach ($getDeviceCategory as $category)
              <div id="tab{{ $category->id }}" class="tabcontent vt-tab-panel">
                <div class="vt-table-wrap">
                  {{-- Rows load one page at a time via /{{ $url_type }}/templates-list-data --}}
                  <table id="datable{{ $category->id }}" class="example table table-striped vt-table" cellspacing="0"
                    data-server-side="1"
                    data-ajax-url="/{{ $url_type }}/templates-list-data"
                    data-category-id="{{ $category->id }}"
                    data-orderable-cols="[1,3,4,5]"
            ...
            @if(Auth::user()->user_type == "Admin")
              {{-- Shared Apply modal: devices load via AJAX select2 (no more full device list per row) --}}
              <div class="modal vt-assign-modal" id="applyTemplateModal" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-md" role="document">
                  <div class="modal-content vt-modal-content">
                    <div class="modal-header vt-modal-header">
                      ...
                      <h4 class="modal-title vt-modal-title"><i class="fa fa-link"></i> Assign template</h4>
                    </div>
                    <div class="modal-body vt-modal-body">
                      <div class="row">
                        <div class="col-md-12">
                          <form id="applyTemplateForm" action="" method="post">
                            @csrf
                            <div class="form-group vt-modal-field">
                              <input type="hidden" name="test_id" id="applyTemplateId" value="">
                              <label class="control-label vt-modal-label" for="applyTemplateDevices">Single / multiple select device</label>
                              <select class="vt-device-select" id="applyTemplateDevices" name="devices[]" multiple style="width:100%;"></select>
                              <p class="help-block vt-modal-hint">Search by IMEI; devices load as you scroll.</p>
                            </div>
...
<script>
  function viewSettingsDataTableOpts($table) {
  ...
  function initViewSettingsDataTable($table) {
    ...
    $table.DataTable(viewSettingsDataTableOpts($table));
  }
  ...
  function openApplyModal(btn) {
    ...
    var $modal = $('#applyTemplateModal');
    if (!$modal.length) return;

    $('#applyTemplateForm').attr('action', '/{{ $url_type }}/assign-template/' + templateId);
    $('#applyTemplateId').val(templateId);

    var $sel = $('#applyTemplateDevices');
    if ($sel.data('select2')) {
    ...
    $sel.select2({
      ...
      ajax: {
        ...
        data: function (params) {
          return { q: params.term || '', page: params.page || 1, category_id: categoryId };
        },
        cache: true
      }
    });
