    ...
    <div class="top-page-header">
      <div class="page-breadcrumb">
        <nav class="c_breadcrumbs">
          <ul>
            <li><a href="#">IMEI Management</a></li>
            <li class="active"><a href="#">View IMEI</a></li>
          </ul>
        </nav>
      </div>
    </div>
            ...
            <table id="esim" class="example table table-bordered table-striped table-condensed cf" style="border-spacing: 0; width: 100%; font-size: 14px;">
              ...
              <tbody>
                ...
                <tr>
                  <td><?php echo $i; ?></td>
                  <td>{{$imei->imei}}</td>
                  <td>{{ CommonHelper::getDateAsTimeZone($imei->created_at) ?? 'N/A'}}</td>
                  <td>{{ CommonHelper::getDateAsTimeZone($imei->updated_at) ?? 'N/A'}}</td>
                  <td>
                    <form action="/{{$url_type}}/delete-esim-customer/{{$imei->id}}" method="post">
                      @csrf
                      @method('DELETE')
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="uploadModalLabel">Upload IMEIS</h5>
